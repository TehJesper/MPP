...
func (s *CharacterService) CreateNewCharacter(
	...
	char := domain.NewCharacter(name, race, class, level, bonus, abilities, modifiers, skills, initiative, passivePerception)
...
func (s *CharacterService) formatCharacter(c domain.Character) string {
	base := fmt.Sprintf(
		`Name: %s
Class: %s
Race: %s
...
Skill proficiencies: %s`,
	...
	if err != nil {
		fmt.Print("This class cannot cast spells")
	}
